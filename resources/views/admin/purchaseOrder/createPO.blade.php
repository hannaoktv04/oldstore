    ...
    <div class="card-body">
        <form action="{{ route('admin.purchase_orders.store') }}" method="POST">
            ...
            <div class="row align-items-end mb-3">
                <div class="col-md-5">
                    ...
                    <select class="form-control" id="item-select">
                        ...
                        @foreach ($items as $item)
                            <option value="{{ $item->id }}" data-kode="{{ $item->kode_item }}" data-nama="{{ $item->nama_barang }}" data-satuan="{{ $item->satuan }}">
                                {{ $item->nama_barang }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-2">
                    <button type="button" class="btn btn-sm btn-outline-primary w-100 " id="add-item"><span class="bi bi-plus"></span> Tambah Item</button>
                </div>
            </div>
