    ...
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" action="{{ route('admin.pengajuan.approve', $pengajuan->id) }}">
            ...
            <div class="modal-content border-0 shadow rounded-4 overflow-hidden"
                style="max-height: 90vh; height: auto; display: flex; flex-direction: column;">
                <div class="modal-header bg-success text-white py-3">
                    <h5 class="modal-title"></i>Pengajuan #{{ $no }}
                    </h5>
                ...
                <div class="modal-body p-4" style="overflow-y: auto;">
                    ...
                    <div class="border rounded-3 p-3 mb-4">
                        ...
                        <input type="number" name="approved_qty[{{ $detail->id }}]" id="approvedQty{{ $detail->id }}"
                            class="form-control" placeholder="Jumlah disetujui" max="{{ $detail->qty_requested }}"
                            min="0" required>
                    </div>
                    @endforeach
                ...
                <div class="modal-footer bg-light px-4 py-3">
                    <button type="submit" class="btn btn-success px-4"> Setujui Pengajuan
                    </button>
                </div>
            </div>
        </form>
