<nav class="navbar navbar-expand-lg bg-white shadow-sm py-2 sticky-top">
    <div class="container d-flex justify-content-between align-items-center">
        ...
        @php
            ...
            $notifikasiProdukBaru =
                Auth::check() && Auth::user()->hasRole('pegawai')
                    ...
                    <div id="cart-popup" class="position-absolute text-dark bg-white shadow rounded p-3 mt-2 z-3 d-none"
                        ...
                        @forelse($cartItems as $item)
                            <div class="d-flex align-items-start mb-2">
                                <img src="{{ asset('storage/' . ($item->item->photo->image ?? 'placeholder.jpg')) }}"
                        ...
                        @if ($notifikasiProdukBaru->count() > 0)
                            <div class="mt-2">
                                <form action="{{ route('notifikasi.markSeen') }}" method="POST">
                                    @csrf
                                    <button class="btn btn-outline-secondary btn-sm w-100" type="submit">Tandai Produk:
                                        Sudah Dibaca</button>
                                </form>
                            </div>
                        @endif
                        ...
                        @if ($notifikasiPengiriman->count() > 0)
                            <div class="mt-2">
                                <a href="{{ route('user.history') }}"
                                    class="btn btn-outline-success btn-sm w-100">Lihat Detail Pengiriman</a>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div id="user-popup" class="position-absolute end-0 mt-2 p-3 rounded shadow bg-white d-none"
                    ...
                    @else
                        ...
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="dropdown-item bg-white text-danger"><i
