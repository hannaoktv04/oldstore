document.addEventListener("DOMContentLoaded", function () {
    const swalAlert = (opts) => Swal.fire(opts);
    ...
    const tableBody  = $("#table-body") || (poTable ? poTable.querySelector("tbody") : null);

    let dataTable = null;
    if (poTable && window.jQuery && jQuery.fn && typeof jQuery.fn.DataTable === "function") {
    ...
    if (itemSelect) {
        itemSelect.addEventListener("change", function () {
            ...
            if (unitInput) unitInput.value = opt?.getAttribute("data-satuan") || "";
    ...
    if (addItemBtn && itemSelect && qtyInput && tableBody) {
        addItemBtn.addEventListener("click", function () {
            ...
            if (dataTable) {
                ...
                tr.innerHTML = rowHtml.map(td => `<td>${td}</td>`).join("");
