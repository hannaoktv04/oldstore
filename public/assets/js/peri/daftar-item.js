document.addEventListener("DOMContentLoaded", function () {
    ...
    $("#filter-kategori, #filter-status, #filter-stok").on(
        "change",
        function () {
            table.draw();
        }
    ...
    $(document).on("click", ".btn-detail", function () {
        ...
        $.get("/admin/items/" + itemId, function (data) {
            ...
            const statusBadge =
                data.state && data.state.is_archived
        ...
        ${photoHtml}
        <table class="table">
          <tr><th style="width:30%;">Nama Produk</th><td>${
              data.nama_barang ?? ""
          }</td></tr>
          <tr><th>Kategori</th><td>${
              data.category ? data.category.categori_name : "N/A"
          }</td></tr>
          <tr><th>Stok Saat Ini</th><td>${
              data.stocks ? data.stocks.qty : 0
          } ${data.satuan ? data.satuan.nama_satuan : ""}</td></tr>
    ...
    const bulkOffcanvas = bulkOffcanvasEl
        ? new bootstrap.Offcanvas(bulkOffcanvasEl)
        : null;
