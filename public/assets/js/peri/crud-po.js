(function () {
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const swalAlert = (opts) =>
            Swal && Swal.fire
                ? Swal.fire(opts)
                : alert(opts?.text || opts || "Alert");
        ...
        const tableBody = poTable
            ? poTable.tBodies[0] || poTable.querySelector("tbody")
            : null;
        ...
        if (itemSelect) {
            itemSelect.addEventListener("change", function () {
                ...
                if (stokAkhirInput)
                    stokAkhirInput.value = opt?.getAttribute("data-stok") || 0;
            });
        }
            ...
            addItemBtn.addEventListener("click", function () {
                ...
                if (isNaN(qtyVal) || qtyVal <= 0) {
                    swalAlert({
                        icon: "error",
                        title: "Oops",
                        text: "Qty harus diisi dan lebih dari 0!",
                    });
                    return;
                }

                const duplicate = $$('input[name="item_id[]"]').some(
                    (i) => i.value === itemId
                );
                if (duplicate) {
                    swalAlert({
                        icon: "error",
                        title: "Duplikat",
                        text: "Item ini sudah ditambahkan!",
                    });
                    return;
                }
